...
def get_examples_split(examples, var):
    ...
    vars = list(set([example[var] for example in
                     examples]))  # use these three lines to identify the two possible values within 'var'
    if len(vars) == 1:
        vars = vars + [
            'dummyvar']  # add on "dummyvar" if a second value isn't found. this wont change any functionality
...
def get_numerical_split(examples, var, target='Class'):
    ...
    vars = list(set([example[var] for example in
                     examples]))  # use these three lines to identify the two possible values within 'var'
    targetvars = list(set([example[target] for example in examples]))

    if len(vars) == 1:
        vars = vars + [
            'dummyvar']  # add on "dummyvar" if a second value isn't found. this wont change any functionality
    if len(targetvars) == 1:
        targetvars = targetvars + ['dummyvar']

    debug_ID3('        -> vars: ',vars)
    debug_ID3('        -> examples: ',examples)
    debug_ID3('        -> var: ',var)
    split0 = [int(example[target] == targetvars[0]) for example in examples if example[var] == vars[0]]
    split1 = [int(example[target] == targetvars[1]) for example in examples if example[var] == vars[1]]
    splits = [split0, split1]
    return splits
...
def best_split(examples, target, vars_to_ignore=[]):
    ...
    for var in variables:  # loop through the variables
        ...
        debug_ID3('    var: ', var)
        debug_ID3('    parent: ', parent_split)
        debug_ID3('    child: ', children_split)
        debug_ID3('    parent entropy: ', parent_entropy)
        debug_ID3('    child entropy: ', children_entropy)
        debug_ID3('    IG: ', information_gain)

        information_gains[information_gain] = var

    maxvalue = max(information_gains.keys())
    bestvariable = information_gains[maxvalue]
    debug_ID3('    -- max value -- : ',maxvalue)
    debug_ID3('    -- best var -- : ', bestvariable)
  ...
  '''
    ...
    while len(fringe) != 0:  # loop until an arbitrary depth limit is reached, or break condition is met
        ...
        if split_variable == "Information Gain was Zero!":
            ...
            child_data, var0, var1 = get_examples_split(current_node.data,
                                                        split_variable)  # result of splitting the data
            ...
            debug_ID3('split by: ', split_variable)
            debug_ID3('child data 0: ', child_data[0])
            debug_ID3('child data 1: ', child_data[1])
...
def test(node, examples):
  ...
  '''
    ...
    for example in examples:
        ...
        score += 1 if true_value == our_guess else 0
    ...
    while len(fringe) != 0:
        ...
        if currentnode.label != "Leaf":  # if node is a leaf node, we don't need to prune it by definition
            ...
            for child in list(children.keys()):
                ...
                debug_Pruning('child examples: ',child_examples)
...
def build_random_forest(Ntrees,examples,default):
    forest = []
    step = int(len(examples)/Ntrees) #amount of data per tree
    for i in range(Ntrees):
        subdata = examples[i*step:(i+1)*step]
        tree = ID3(subdata,default)
        forest.append(tree)
    return forest
