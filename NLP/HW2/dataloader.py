...
class wiki_dataloader(pl.LightningDataModule):
    ...
    def __init__(self, datasets, batch_size, num_workers=2, unk_threshold=1.):
        ...
        self.train = [[each[0],each[1]] for each in self.train_dataset if torch.sum(each[0]==datasets[0].token_map['<unk>']) <= int(unk_threshold*datasets[0].window)]
        self.val = [[each[0],each[1]] for each in self.valid_dataset if torch.sum(each[0]==datasets[0].token_map['<unk>']) <= int(unk_threshold*datasets[0].window)]
        self.test = [[each[0],each[1]] for each in self.test_dataset if torch.sum(each[0]==datasets[0].token_map['<unk>']) <= int(unk_threshold*datasets[0].window)]
